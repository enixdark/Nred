$(document).ready(function() {
	...
	if(channel !== ""){
		console.log("Trying to create of Join channel", channel);
		socket.emit('create or join', channel);
	}

	socket.on('created',function(channel){
		console.log('channel ' +  channel + ' has been created');
		console.log('This is peer is the inittiator');

		div.insertAdjacentHTML('beforeEnd', '<p>Time: ' + ( performance.now() / 1000).toFixed(3) + ' ---> Channel' + channel + 'has been create </p>');
		div.insertAdjacentHTML('beforeEnd', '<p>Time: ' + ( performance.now() / 1000).toFixed(3) + ' ---> This peer is the inittitor ... </p>');
	});

	socket.on('full',function(channel){
		console.log('channel' + channel + 'is too crowded! Cannot allow tou to enter, soory:-(');
		div.insertAdjacentHTML( 'beforeEnd' , '<p>Time: ' + 
			( performance . now () / 1000 ). toFixed ( 3 ) + 
			' --> channel ' + channel + ' is too crowded! Cannot allow you to enter, sorry :-( </p>' );
	});
	...
	socket.on('boardcast: joined',function(msg){
